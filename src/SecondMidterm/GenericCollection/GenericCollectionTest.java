...
class GenericCollection<T extends Comparable<T> & IHasTimestamp> {
    private HashMap<String, TreeSet<T>> itemsByCategory;
    private TreeMap<LocalDateTime, T> itemsByTimestamp;

    public GenericCollection() {
        this.itemsByCategory = new HashMap<>();
        this.itemsByTimestamp = new TreeMap<>();
    }

    void addGenericItem (String category, T element) {
        itemsByCategory.putIfAbsent(category, new TreeSet<>());
        itemsByCategory.computeIfPresent(category, (k, v) -> {
            v.add(element);
            return v;
        });

        itemsByTimestamp.putIfAbsent(element.getTimestamp(), element);
    }

    public Collection<T> findAllBetween(LocalDateTime from, LocalDateTime to) {
        TreeSet<T> tmp = new TreeSet<>(itemsByTimestamp.subMap(from, to).values());
    ...
    public Collection<T> itemsFromCategories(List<String> categories) {
        TreeSet<T> tmp = categories.stream()
               .flatMap(c -> itemsByCategory.get(c).stream())
    ...
    public Map<String, Set<T>> byMonthAndDay() {
        return itemsByTimestamp.entrySet()
                .stream()
                .collect(Collectors.toMap(
                        (e -> String.format("%02d-%02d", e.getKey().getMonthValue(), e.getKey().getDayOfMonth())),
                        e -> {
                            TreeSet<T> value = new TreeSet<>(Comparator.reverseOrder());
